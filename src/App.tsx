        ...
        <div className="max-w-4xl mx-auto search-container">
          <div className="grid md:grid-cols-3 gap-4">
            ...
            <input
              type="text"
              value={searchTerm}
              onChange={(e) => handleDebouncedSearch(e.target.value, selectedPracticeArea, selectedLocation)}
          ...
          {!isSearchActive && (
            <div className="text-center mt-4">
              <p className="text-gray-600 text-sm">
                Use the search above to find lawyers by name, practice area, or location
              </p>
            </div>
          )}
          ...
          {isSearchActive && (
            <div className="text-center mt-4">
              ...
              >
                Clear Search & Show Featured Lawyers
              </button>
            </div>
          )}
        </div>
...
function App() {
  ...
  const [displayedLawyers, setDisplayedLawyers] = useState<Lawyer[]>(initialLawyers.slice(0, 3)); // Featured lawyers only
  const [selectedLawyer, setSelectedLawyer] = useState<Lawyer | null>(null);
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedPracticeArea, setSelectedPracticeArea] = useState('');
  const [selectedLocation, setSelectedLocation] = useState('');
  const [isSearchActive, setIsSearchActive] = useState(false); // Track if user is searching
  ...
  const handleSearch = React.useCallback(() => {
    const hasActiveSearch = searchTerm || selectedPracticeArea || selectedLocation;
    setIsSearchActive(hasActiveSearch);
    
    // If no search criteria, show only featured lawyers
    if (!hasActiveSearch) {
      setFilteredLawyers(initialLawyers);
      setDisplayedLawyers(initialLawyers.slice(0, 3)); // Only 3 featured lawyers
      setCurrentPage(1);
      return;
    }
    ...
    setDisplayedLawyers(filtered.slice(0, 12)); // Show more results when searching
    setCurrentPage(1);
  }, [searchTerm, selectedPracticeArea, selectedLocation, lawyers, initialLawyers]);
  ...
  const loadMoreLawyers = React.useCallback(() => {
    if (isLoadingMore || !isSearchActive) return; // Only allow load more during search
  ...
  }, [currentPage, filteredLawyers, isLoadingMore, isSearchActive]);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {/* Load More Trigger */}
        {isSearchActive && displayedLawyers.length < filteredLawyers.length && (
        ...
        {filteredLawyers.length === 0 && isSearchActive && (
